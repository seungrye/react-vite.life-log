import { addDoc, doc, collection, getDocs, query, updateDoc, where } from "firebase/firestore";
 ...
 * 
 */
async function migrate_post_to_post_v2() {
...
export function migrateFirestore() {
  ...
  const migrateFnList = [migrate_post_to_post_v2];
  ...
  const applyMigrate = async (fn: (() => void)) => {
    try {
      fn?.();
    } catch (e: unknown) {
      console.error(e)
